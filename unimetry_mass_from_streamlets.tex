...
\subsection{Mass as a functional of the spatial second moment \texorpdfstring{\(\mathbf C_B\)}{C\_B}}
...
where
\begin{itemize}
  \item \(\kappa_*\) is a universal scale factor,
  \item \(F\) is a dimensionless structural functional such that
  \begin{itemize}
    \item \(F(0,\cdot,\cdot)=0\) (no spatial content \(\Rightarrow\) no mass),
    \item \(F\) is monotone in \(I_1\),
    \item dependence on \(I_2,I_3\) encodes shape/anisotropy.
  \end{itemize}
\end{itemize}
...
\begin{itemize}
  \item If \(\operatorname{tr}\mathbf C_B = 0\), then \(m_0 = 0\): no spatial loops, no mass.
  \item For more complex objects, \(\operatorname{tr}\mathbf C_B\) grows with the amount of spatial circulation in the streamlet ensemble, and so does \(m_0\).
\end{itemize}
...
\begin{quote}
Rest mass is a scalar functional of the spatial second moment \(\mathbf C_B\), i.e.\ of the amount and geometry of spatial streamlet projections, not an input parameter used to define \(\zeta\).
\end{quote}


\subsection{Energy and D-boosts: recovering \texorpdfstring{\(E = \gamma m_0 c^2\)}{E = γ m\_0 c²}}

In \texttt{Energy.tex}, the phase–space energy density is written as
\[
...
\begin{itemize}
  \item the internal quantities \(\zeta, \mathbf C_B, k, \dot H\) are invariant under D-boosts,
  \item only the decomposition of 4-velocity into temporal/spatial parts changes.
\end{itemize}
...
emerges with:
\begin{itemize}
  \item \(m_0\) derived purely from the spatial structure of the streamlet ensemble via \(\mathbf C_B\) and \(\zeta\),
  \item \(c^2\) as the invariant scale set by the calibrated flow rate \(\dot H\),
  \item \(\gamma\) as the familiar boost factor coming solely from the D-rotation between temporal axes, with no change in the internal structure.
\end{itemize}
