...
\subsection{Hopf fibration and the phase parameter $\chi$}
\label{subsec:hopf-fibration-chi}

In addition to the external orientation data $(p_a,N_a)$, a streamlet carries an
intrinsic cyclic degree of freedom describing its phase state.  We denote
this phase parameter by $\chi$, to be understood as an angle variable with the
identification $\chi\sim\chi+2\pi$.


\paragraph{Local $S^3$ around a point defect.}
Fix a streamlet defect at $p_a\in\Espace$.  For sufficiently small $\varepsilon>0$,
the boundary of the $\delta$--ball $B_\varepsilon(p_a)$ is a $3$--sphere
\[
   \partial B_\varepsilon(p_a) \cong S^3.
\]
After translation by $-p_a$ and radial normalization, we identify this $S^3$
with the unit quaternions
\[
   S^3 \;\cong\; \{\,q\in\mathbb H:\ |q|=1\,\},
\]
where $\mathbb H=\mathrm{span}\{1,\mathbf i,\mathbf j,\mathbf k\}$.

\paragraph{Hopf projection (base vector).}
Fix the imaginary unit $\mathbf i\in\mathrm{Im}\,\mathbb H$.  The Hopf map
$\pi:S^3\to S^2$ is given by
\begin{equation}\label{eq:hopf-map}
   \pi(q) \;:=\; q\,\mathbf i\,q^{-1}
   \;\in\; S^2 \subset \mathrm{Im}\,\mathbb H.
\end{equation}
For a pure streamlet, the Hopf base vector is precisely the image
\[
   \mathbf u_a = \pi(q_a)\in S^2,
\]
for some lift $q_a\in S^3$.  Allowing $\|\mathbf u_a\|<1$ may be interpreted as a
coarse--grained mixture of nearby pure base directions.

\paragraph{Fiber and the cyclic phase $\chi$.}
The Hopf fiber over a fixed base vector $\mathbf u\in S^2$ is an $S^1$ orbit.
Concretely, if $q_0\in S^3$ satisfies $\pi(q_0)=\mathbf u$, then the entire fiber is
\begin{equation}\label{eq:hopf-fiber}
   \pi^{-1}(\mathbf u)
   = \{\, q(\chi) = q_0\,e^{\mathbf i\chi} \;:\; \chi\in[0,2\pi)\,\},
\end{equation}
where $e^{\mathbf i\chi}=\cos\chi+\mathbf i\sin\chi$.
Since $e^{\mathbf i(\chi+2\pi)}=e^{\mathbf i\chi}$, the parameter
\begin{equation}\label{eq:chi-periodic}
   \chi \sim \chi + 2\pi
\end{equation}
is \emph{cyclic}.  Moreover, the Hopf base vector is invariant along the fiber:
\[
   \pi\!\big(q(\chi)\big)=\pi(q_0)=\mathbf u.
\]
We therefore interpret $\chi$ as the intrinsic \emph{phase parameter} of a
streamlet state: varying $\chi$ moves the streamlet along its internal phase
cycle while returning it to the same state after $\Delta\chi=2\pi$.

\paragraph{Why a base vector appears.}
The defect is compact (a point in $4$D), hence its small boundary is the compact
$3$--sphere $S^3$.  The Hopf fibration decomposes this $S^3$ into a family of
closed circles (fibers) parametrized by a base point on $S^2$.  Selecting a
particular closed phase cycle around the defect therefore selects a unique base
direction $\mathbf u\in S^2$ (or, after coarse--graining, a vector
$\mathbf u\in\overline{B^3}$).  This is the geometric origin of the streamlet
base vector $\mathbf u_a$.
