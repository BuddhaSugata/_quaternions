...
(a unit vector field, $\delta(N,N)=1$).  The associated time 1--form
...
The minimal observer input is a choice of a coherent unit axis $N$ (equivalently,
...
by a coherent unit axis $N$ and its time 1--form $\alpha$, and interpret the
...
\begin{itemize}
  ...
  axis and $N$ as a coherent time axis).  The induced Lorentzian form
  ...
  \item Section \ref{sec:flow-invariants-S3} reformulates 4--velocity normalization
...
coherent time axis).  For a single inertial observer one may, and often will,
...
% ------------------------------------------------------------
\subsection{Coherent time axis $N$ and the induced Lorentzian form}
\label{subsec:RSS}
...
Relative to the coherent time axis $N$, write the observer split
\[
...
The construction starts from an oriented coherent observer axis $N$
...
\emph{proto--affine}) if the associated proto--velocity
\[
...
time variable; operational time is introduced via the coherent axis $N$ below.
\end{remark}
...
orientation of $N$ (a future--pointing branch) and keep it coherent on the region
under consideration.
\end{remark}
...
Given an oriented coherent axis $N$, the time form $\alpha$ defines the observer--adapted
...
\item $t$ is the observer--adapted coordinate time obtained by projecting $dX$ onto the
      coherent axis $N$ via \eqref{eq:dt-def};
\item $\tau$ is the proper time of an ideal comoving clock, defined invariantly as the
      $g$--arc length by \eqref{eq:tau-def}.
\end{itemize}
Operationally, the observer has access to $t$ (relative to its chosen coherent axis $N$)
...
% ===============================
\section{Protovelocity invariants and the emergence of $S^3$ in the Euclidean proto-space}
\label{sec:flow-invariants-S3}
% ===============================

This section formalizes a central equivalence of the proto--space approach.
Once a coherent observer axis $N$ (hence the Lorentzian form $g$) is fixed,
the standard SR normalization of the 4--velocity in proper time,
\[
   g(\dot X,\dot X)=\clight^2,
\]
is equivalent (up to reparameterization) to a constant Euclidean protovelocity budget
in proto--space,
\[
   \delta(\widetilde X,\widetilde X)=\clight^2,
\]
with respect to a calibrated proto--parameter $\chi$.
The latter condition forces the instantaneous protovelocity states to lie on the Euclidean sphere
$S^3_{\clight}\subset T_p\Espace$.

% ------------------------------------------------------------
\subsection{Worldlines, proto--parameters, and the protovelocity}
...
nowhere--vanishing derivative.  The associated \emph{protovelocity} is
...
This calibration is auxiliary: it fixes the \emph{total} Euclidean protovelocity budget
in $\Espace$ but is not, by itself, the observer's operational time variable.
\end{remark}

In words: in a $\delta$--calibrated proto--parameter, the protovelocity $\widetilde X$
...
(with a chosen time orientation).  Pointwise along $X$, decompose the protovelocity
...
\begin{lemma}[Euclidean and Lorentzian norms of the protovelocity]
...
Motivated by the phase/clock viewpoint, define the \emph{interval rate} with respect
to $\chi$ by
...
% ------------------------------------------------------------
\subsection{Equivalence: proper--time normalization $\Longleftrightarrow$ calibrated protovelocity}
...
Statement (B) is the corresponding calibration of the proto--space protovelocity.
...
% ------------------------------------------------------------
\subsection{The $S^3$ of admissible protovelocity states}
...
While the calibrated protovelocity states form the full sphere $S^3_{\clight}(p)$,
...
\paragraph{Calibrated proto--flows and circular angle data.}
A $\chi$--calibrated proto--flow state is a vector $\tilde X\in T_p\Espace$
...
\[
  ...
  \tilde H^2+\tilde L^2=\clight^2,
\]
and define the circular tilt $\xi\in[0,\pi]$ by
\begin{equation}\label{eq:xi-def}
  \cos\xi:=\frac{\tilde H}{\clight},
  \qquad
  \sin\xi:=\frac{\tilde L}{\clight}.
\end{equation}
The observer speed parameter (measured in the observer coordinate time $t$) is
\begin{equation}\label{eq:beta-def-xi}
  \beta:=\frac{v}{\clight}=\frac{\tilde L}{\tilde H}=\tan\xi.
\end{equation}
...
From \eqref{eq:U-from-tildeX-xi} and \eqref{eq:xi-def} one obtains purely in terms of $\xi$
...
\begin{equation}\label{eq:R-xiE}
  ...
  \quad\in\Spin(1,3)\subset\Cl^+(4,0),
\end{equation}
so that
\[
...
If one replaces $U(\xi,E)$ in \eqref{eq:R-xiE} by the unrescaled proto--flow direction
...
defines a proto--flow direction $\cos\xi\,(\text{time})+\sin\xi\,(\text{space})$ on
...
an observer chooses a coherent time axis $N$ and hence a time 1--form $\alpha$.
...
it: departures from inertial kinematics correspond to variations of the coherent
...
(coherently constant) "time" axis represented by a field vector $N$ with
...
the coherent time direction, rather than by introducing a Lorentzian metric as
a primitive object.
...
\end{equation}
Thus a radial variation of the coherent time axis produces the characteristic
