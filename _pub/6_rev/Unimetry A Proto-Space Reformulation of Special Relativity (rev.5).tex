...
\subsection{Observer datum and unimetry decomposition}
\label{subsec:distN}
Let $N$ be a smooth $\delta$--unit vector field on $\Espace$ satisfying
\[
   \delta(N,N)=1.
\]
In the special--relativistic (inertial) regime studied in this paper we work
locally and take $N$ to be constant on the region of interest (equivalently,
$\nabla^\delta N=0$ with respect to the flat Euclidean connection).  The role
of $N$ is to encode an \emph{observer splitting} (a $1{+}3$ decomposition) of
the Euclidean proto--space.
...
\paragraph{Unimetry decomposition.}
For any $X\in T_p\Espace$ we define the $\delta$--orthogonal splitting
\begin{equation}\label{eq:HL-split}
   H := (X\!\cdot\!N)\,N,\qquad
   L := X-H \;=\; h(X),
\end{equation}
so that $H\parallel N$ and $L\perp_\delta N$.  In the intended interpretation,
$H$ is the internal (temporal) portion of the same calibrated flow $X$ with
respect to the observer axis $N$, while $L$ is the external (spatial) portion.
The point of the present formalism is that a \emph{single} Euclidean flow $X$
admits different $(H,L)$ decompositions for different choices of observer datum
$N$.

\paragraph{Circular parametrization by $\sin/\cos$.}
Fix $\clight>0$ and consider the calibrated sphere
\[
   S^3_{\clight}:=\{X\in T_p\Espace:\ \|X\|_\delta=\clight\}.
\]
For $X\in S^3_{\clight}$ define the \emph{tilt angle} $\vartheta\in[0,\pi]$ by
\begin{equation}\label{eq:theta-sincos}
   \cos\vartheta := \frac{X\!\cdot\!N}{\clight},
   \qquad
   \sin\vartheta := \frac{\|L\|_\delta}{\clight}
   \quad\text{with }L:=X-(X\!\cdot\!N)N.
\end{equation}
If $L\neq 0$ set $E:=L/\|L\|_\delta\in N^{\perp_\delta}$; then
\begin{equation}\label{eq:X-circle}
   X=\clight\big(\cos\vartheta\,N+\sin\vartheta\,E\big).
\end{equation}
This makes the underlying circular geometry explicit: for any fixed unit
$E\perp_\delta N$, the orthogonal map $A(\vartheta)\in O(4)$ acting as a
rotation by $\vartheta$ in the plane $\mathrm{span}\{N,E\}$ (and as the identity
on its orthogonal complement) satisfies $A(\vartheta)N=\cos\vartheta\,N+\sin\vartheta\,E$,
hence $\vartheta$ parametrizes the full circle in that plane.  In particular,
$\vartheta=\pi/2$ corresponds to $X\!\cdot\!N=0$, i.e.\ vanishing $N$--projection
($H=0$ in \eqref{eq:HL-split}); this will later be the limit in which the
observer assigns no internal (temporal) component to the flow.

\begin{remark}[Status of $N$ and absence of a preferred inertial frame]
\label{rem:N-status}
In the SR part of the paper, $N$ is not postulated as a dynamical ``aether''
field and is not assigned field equations, an action, or stress--energy.
Rather, it is an \emph{observer datum} required to define the $1{+}3$ splitting
\eqref{eq:HL-split} and the induced Lorentzian bilinear form introduced below.

The construction is $\mathrm{O}(4)$--equivariant: for any Euclidean isometry
$A\in \mathrm{O}(4)$, the simultaneous transformation $(X,N)\mapsto (AX,AN)$
preserves all scalar relations derived from the construction.  In particular,
the Lorentzian form $g_N$ defined in \S\ref{subsec:LMD} satisfies
$g_{AN}(AX,AY)=g_N(X,Y)$.  Thus a constant choice of $N$ does not represent a
physically preferred direction; it is a label for an inertial description.

A possible global/dynamical reading of $N$ as an effective field associated
with sources (e.g.\ in a GR extension or in a ``3--sphere source'' picture)
is conceptually distinct and not developed in the present SR--focused paper.
Within SR we restrict to local inertial patches where $N$ may be taken constant.
\end{remark}

\begin{remark}[Two--body formulation without a background]
\label{rem:two-body-no-background}
When no universal background is assumed, it is natural to take the observer
axis from a physical reference object.  For instance, if a body $B$ carries a
calibrated flow $X_B\in S^3_{\clight}$, one may set $N_B:=X_B/\clight$ and decompose
another flow $X_A$ as $X_A=H_{A|B}+L_{A|B}$ by \eqref{eq:HL-split} with $N=N_B$.
In this way, ``relative motion'' is represented as a change of the observed
split of the \emph{same} Euclidean flow into temporal and spatial portions.
\end{remark}
...
\begin{equation}\label{eq:g-def-proj}
   g_{AB} = N_A N_B - h_{AB}.
\end{equation}
The definition is invariant under $N\mapsto -N$, since $N_A N_B=(-N)_A(-N)_B$.
...
\begin{lemma}[Unimetry identity on $S^3_{\clight}$]
\label{lem:unimetry}
Let $X\in S^3_{\clight}$ and let $X=H+L$ be the decomposition
\eqref{eq:HL-split}.  Then
\[
   g(X,X)=\|L\|_\delta^2-\|H\|_\delta^2.
\]
Equivalently, with $\vartheta$ defined by \eqref{eq:theta-sincos},
\[
   \|H\|_\delta=\clight\cos\vartheta,\qquad
   \|L\|_\delta=\clight\sin\vartheta,\qquad
   g(X,X)=\clight^2\cos(2\vartheta).
\]
\end{lemma}

\begin{proof}
Using $X=H+L$ with $H\parallel N$ and $L\perp_\delta N$, one has
$g(H,L)=0$, $g(H,H)=\|H\|_\delta^2$, and $g(L,L)=-\|L\|_\delta^2$, hence
$g(X,X)=g(H,H)+g(L,L)=\|H\|_\delta^2-\|L\|_\delta^2$, which is the first claim
(up to the displayed sign convention).  The trigonometric identities follow
immediately from \eqref{eq:theta-sincos}.
\end{proof}

\paragraph{Ratio parametrization (introduced later).}
On the domain $\cos\vartheta\neq 0$ (in particular, on the timelike sector
identified later) one may equivalently use the ratio parameter
\begin{equation}\label{eq:beta-tan}
   \beta := \tan\vartheta = \frac{\|L\|_\delta}{\|H\|_\delta},
\end{equation}
with $\sin\vartheta=\beta/\sqrt{1+\beta^2}$ and $\cos\vartheta=1/\sqrt{1+\beta^2}$.
We delay the use of $\beta$ because $\vartheta$ provides the global circular
parametrization of the $2$--plane geometry, including the limiting case
$\vartheta=\pi/2$ where $\beta$ diverges.
...
\]
Then the Lorentzian metric constructed above admits the equivalent
closed form
...
onto $N^{\perp_\delta}$ (so $X_\perp=L$ in \eqref{eq:HL-split}).
\end{proposition}
