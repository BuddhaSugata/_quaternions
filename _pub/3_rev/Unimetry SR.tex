...
\maketitle
\begin{abstract}
We recast special-relativistic kinematics as geometry of a single phase–flow variable on a quaternionic slice $\mathbb C_{\mathbf u}\subset\mathbb H$.
A unit rotor $\widehat q(\vartheta,\mathbf u)=\cos\vartheta+\mathbf u\sin\vartheta$ encodes motion by a circular angle $\vartheta$, establishing the dictionary
$\beta=\sin\vartheta$, $\gamma=\sec\vartheta$, and
$k_{\rm Doppler}=\frac{1+\tan(\vartheta/2)}{1-\tan(\vartheta/2)}$.
We prove that phase-derivative invariants coincide with Minkowski invariants; longitudinal Doppler and velocity addition follow from rotor composition; and non-collinear compositions reproduce the Wigner rotation and Thomas precession. Photons appear as null proper-time flows with the phase parameter $\chi$ serving as an affine counter.

Extending to curved backgrounds, we introduce a gravitational clock angle $\phi$ via the tetrad time leg ($\cos\phi=\sqrt{-g_{00}}$ in stationary cases) and obtain a unified time-rate factorization
$\displaystyle d\tau/dt=\cos\phi\,\cos\vartheta$,
which cleanly separates gravitational and kinematic effects and admits an optical-length representation of time-of-flight, including the Shapiro delay. The formulation is strictly equivalent to SR/GR kinematics yet replaces hyperbolic rapidities by compact phase angles, unifying rotations and boosts within a single rotor calculus. Besides conceptual economy, the circular parametrization improves numerical behavior near $|\beta|\to 1$ and simplifies symbolic manipulations.
\end{abstract}


\paragraph{Keywords:} special relativity; phase; rapidity; Doppler shift; Lorentz factor; Wigner rotation; Thomas precession; effective medium; optics; phase parametrization.
...
\]
We use $\clight$ for the speed of light; $\beta:=\mathbf v/\clight$, $\gamma:=1/\sqrt{1-\beta^2}$, and rapidity defined by $\tanh\eta=\beta$. The subscript $l$ in $dx_l$ denotes spatial components, with $l=1,2,3$ a Cartesian index.
\begin{table}[t]
\centering
...
In the flow-based description an object is a collection of elementary \textbf{streamlets} (fragments of total object phase flow). Each streamlet admits a co-moving frame in which its spatial projection vanishes and its budget angle equals zero in that frame (\textbf{self time} direction).
...
\emph{Cyclical postulate}. From the ability of bodies to preserve a place within the observable space, we conclude the cyclical nature of streamlets' flows in $\mathcal E$:
...
With these ensemble quantities we define the body's proper time
increment by
\begin{equation}
d\tau_B:=\sqrt{T_B}\, d\chi,
\label{eq:body-proper-time}
\end{equation}
and the body's self spatial quadratic form on $S$ by
\begin{equation}
q_B(d\mathbf \ell):= d\mathbf \ell^{\!\top}\mathbf C_B\,d\mathbf \ell .
\label{eq:body-spatial-form}
\end{equation}
This yields a natural candidate for the body's self metric as a
...
\end{equation}
Thus $\clight$ is not introduced dynamically but arises as the fixed magnitude of the
...
\label{eq:zeta-def}
\end{equation}
We call $\zeta$ the effective (intrinsic) angle: it aggregates the temporal/spatial budget at the level relevant for the intrinsic metric \eqref{eq:body-self-metric}. In particular,
\[
...
A used mnemonic for a nonvanishing spatial projection is to pair opposite tilts
\[
...
dynamics but merely a parametrization of the second moments:
...
set $\zeta=\phi=0$ and use $dt:=d\chi$; then the kinematic angle $\vartheta$ is the
...
define the observer–object angle by
\[
\widehat{\mathbf F}
...
\end{equation}
The proof of Minkowski and phase invariants' equivalence might be found in \S\ref{subsec:phase-minkowski-equality}.
...
Quaternions form the minimal non-commutative algebra that (i) double-covers $SO(3)$ for rigid 3D rotations; (ii) carries the Hopf fibration $S^3\to S^2$, separating an internal $S^{1}$ time fiber from spatial orientations; and (iii) encodes the non-commutativity responsible for Wigner–Thomas rotations (residual spatial rotations from composing non-collinear boosts) \cite{Wigner1939,Hopf1931,Thomas1926}.
...
\end{equation}
and a spatial rotation is an R-rotation
...
\end{equation}
Kinematic mapping: $\beta\equiv \mathbf v/ \clight=\sin\psi$, $\gamma=1/\cos\psi$, $\displaystyle \tan\frac{\psi}{2}=\frac{\gamma\beta}{\gamma+1}$. For quaternionic/GA accounts of rotors and Lorentz boosts see \cite{DoranLasenby2003,Hamilton1844,HestenesSobczyk1984}.
...
\begin{enumerate}\itemsep4pt
\item $U(1)$ fiber by right action.
...
\item Surjectivity and $S^2$ base.
...
\item  Link to quantum spheres.
Identifying $\Im\mathbb H\simeq\mathfrak{su}(2)$ (and, via Pauli matrices, with $\mathbb R^3$), the map 
\[
\mathbf n(q)=q\,K\,q^{-1}
\]
is the quaternionic form of the Bloch vector for a qubit, or the Stokes vector on the Poincaré sphere for polarization: normalized state rays $(\psi\in\mathbb C^2,\ \|\psi\|=1)$ modulo global phase $U(1)$ correspond to points on $S^2$. Thus $S^3/U(1)\cong S^2$ is the familiar “state-ray” geometry; our time fiber is the same $U(1)$ that is unobservable as a global phase in standard QM, but becomes a kinematic degree of freedom here.

\item  Geometric phase.
If $q(\tau)$ traces a loop whose shadow $\mathbf n(\tau)$ makes a closed path on $S^2$, parallel transport in the $U(1)$ fiber accumulates a Berry‐type phase $\gamma=\tfrac{1}{2}\Omega[\mathbf n]$, half the solid angle enclosed by the loop on the sphere. In our setting this is an internal time holonomy: purely kinematic and arising from the bundle connection induced by the Hopf fibration.
\end{enumerate}

\paragraph{Remark.}
Left–conjugation $v\mapsto q\,v\,q^{-1}$ rotates lab vectors (changes the base point on $S^2$), whereas right multiplication by $e^{\frac{\sigma}{2}K}$ moves along the fiber at fixed $\mathbf n$. This cleanly separates spatial reorientation from internal (time/phase) evolution.
...
We will systematically replace hyperbolic functions and nested square roots by the circular trigonometry of a single phase angle $\vartheta$, interpreting $\cos\vartheta$ as the temporal projection and $\sin\vartheta$ as the spatial one. This keeps all kinematic identities while avoiding hyperbolic parametrization.  

\emph{Convention.} In this section we track a single body, so $\tau\equiv\tau_B$.

We define the kinematic angle $\vartheta\in[0,\tfrac{\pi}{2})$ of a system with respect to a fixed observer as an \textbf{operationally measurable split} of a constant ``flow budget'' $\clight$ between temporal and spatial projections:
\begin{equation}
...
The phase angle $\vartheta$ is \textbf{not} the rapidity $\eta$; they are related by a Gudermann-type bridge (~\ref{app:gd-bridge})~\cite{Gudermann1830}
\begin{equation}
...
\end{equation}
which implies Einstein's velocity addition \cite{Rindler2006}
\begin{equation}
...
\end{equation}
where $R_W\in\mathrm{SO}(3)$ is the Wigner--Thomas rotation (a genuine 3D rotation), while $q(\vartheta_{12},\mathbf u_{12})$ is the effective boost in the slice $\mathbb C_{\mathbf u_{12}}$. The mechanism can be found refined in \ref{subsec:wigner}.  The rotation angle and axis can be extracted from the vector part of the quaternion product; a closed expression equivalent to the standard formulas is provided in Appendix~\ref{app:1} \cite{Wigner1939,Thomas1927}.
...
\bottomrule
\end{tabular}
\caption{One-to-one correspondences between hyperbolic (rapidity) and circular (phase) parametrizations; cf.\ \cite{Rindler2006} for rapidity and for the longitudinal Doppler \cite{Jackson1998,Schwartz1987}.}
...
We represent a flow state on a quaternionic complex slice
$\mathbb C_{\mathbf u}:=\mathrm{Span}\{1,\mathbf u\}\subset\mathbb H$,
...
\]
\noindent This parametrization is standard in the rotor/GA literature \cite{Hestenes2015,DoranLasenby2003}.
...
the ray \cite{Rindler2006}. With the calibration \eqref{eq:calib-null} we have
\begin{equation}
...
\end{equation}
with the upper sign for receding and lower for approaching.
Equation \eqref{eq:doppler-inf-long} is the phase–derivative Doppler law.
...
\end{equation}
\noindent Cf.\ the Gordon optical metric \cite{Gordon1923}.
...
\begin{itemize}\itemsep4pt
  ...
  \item \textbf{Shapiro delay (gravitational contribution to TOF):} with \(\zeta=0,\ \vartheta=0\), \eqref{eq:optical-length} yields \(t_\Gamma=(1/\clight)\int \sec\phi\,ds\); the excess over \(\int ds/\clight\) is the Shapiro time delay \cite{Shapiro1964}.
...
\begin{equation}
\cos\phi\ :=\ e^0{}_\mu\,u^\mu\ =\ \frac{d\tau_{\rm stat}}{dt}\ =\ \sqrt{-g_{00}}\qquad\text{(stationary case)} \cite{Rindler2006}.
...
H.~Minkowski, ``Raum und Zeit,'' \emph{Physikalische Zeitschrift} \textbf{10} (1909) 104--111. 
English translation: \emph{Space and Time}, in \textit{The Principle of Relativity}, Dover (1952), 75--91.

\bibitem{Rowe2009}
D.~E.~Rowe, ``A Look Back at Hermann Minkowski's Cologne Lecture `Raum und Zeit','' \emph{The Mathematical Intelligencer} \textbf{31} (2009) 27--39. 
\href{https://doi.org/10.1007/s00283-009-9027-0}{doi:10.1007/s00283-009-9027-0}.

\bibitem{Rindler2006}
W.~Rindler, \emph{Relativity: Special, General, and Cosmological}, 2nd ed., Oxford University Press, Oxford (2006). 
ISBN 978-0-19-856731-8.

\bibitem{Hestenes2015}
D.~Hestenes, \emph{Space-Time Algebra}, 2nd ed., Birkh{\"a}user, Cham (2015). 
\href{https://doi.org/10.1007/978-3-319-18413-5}{doi:10.1007/978-3-319-18413-5}.

\bibitem{DoranLasenby2003}
C.~Doran and A.~Lasenby, \emph{Geometric Algebra for Physicists}, Cambridge University Press, Cambridge (2003). 
\href{https://doi.org/10.1017/CBO9780511807497}{doi:10.1017/CBO9780511807497}.

\bibitem{Jackson1998}
J.~D.~Jackson, \emph{Classical Electrodynamics}, 3rd ed., Wiley, New York (1998). 
ISBN 978-0471309321.

\bibitem{Schwartz1987}
M.~Schwartz, \emph{Principles of Electrodynamics}, Dover, New York (1987; unabridged reprint of 1972 McGraw-Hill ed.). 
ISBN 978-0486654935.

\bibitem{Wigner1939}
E.~P.~Wigner, ``On Unitary Representations of the Inhomogeneous Lorentz Group,'' \emph{Annals of Mathematics} \textbf{40} (1939) 149--204. 
\href{https://doi.org/10.2307/1968551}{doi:10.2307/1968551}.

\bibitem{Thomas1927}
L.~H.~Thomas, ``The Kinematics of an Electron with an Axis,'' \emph{Philosophical Magazine} (Ser.~7) \textbf{3} (1927) 1--22. 
\href{https://doi.org/10.1080/14786440108564170}{doi:10.1080/14786440108564170}.

\bibitem{Gordon1923}
W.~Gordon, ``Zur Lichtfortpflanzung nach der Relativit{\"a}tstheorie,'' \emph{Annalen der Physik} \textbf{377} (1923) 421--456. 
\href{https://doi.org/10.1002/andp.19233772202}{doi:10.1002/andp.19233772202}.
...
\section{Extended SR–phase correspondences and the Gudermannian bridge}
\label{app:gd-bridge}
We collect frequently used identities that relate the standard hyperbolic (rapidity \(\eta\)) and the circular phase picture (angle \(\vartheta\)).
Throughout we set
\begin{equation}
\boxed{\;\vartheta:=\operatorname{gd}(\eta)\;}\qquad\Longleftrightarrow\qquad
\sin\vartheta=\tanh\eta,\quad \cos\vartheta=\operatorname{sech}\eta,\quad \tan\vartheta=\sinh\eta.
\end{equation}

\subsection*{Table of correspondences}
...
Quantity & Standard SR (hyperbolic) & Phase picture (circular) \\
\hline
Rapidity & \(\eta\) & \(\vartheta=\operatorname{gd}(\eta)\) \\
Lorentz factor & \(\gamma=\cosh\eta\) & \(\gamma=\sec\vartheta\) \\
Speed & \(\beta=\tanh\eta\) & \(\beta=\sin\vartheta\) \\
Temporal projection & \(\gamma^{-1}=\operatorname{sech}\eta\) & \(\cos\vartheta\) \\
Spatial factor & \(\beta\gamma=\sinh\eta\) & \(\tan\vartheta\) \\
Half-parameters & \(\tanh\!\frac{\eta}{2}\) & \(\tan\!\frac{\vartheta}{2}\) \\
Longitudinal Doppler & \(k=e^{\pm\eta}\) & \(k=\dfrac{1+\tan(\vartheta/2)}{1-\tan(\vartheta/2)}\) \\
Energy & \(E=m\cosh\eta\) & \(E=m\,\sec\vartheta\) \\
Momentum & \(p=m\sinh\eta\) & \(p=m\,\tan\vartheta\) \\
Velocity addition & \(\tanh(\eta_1\!\pm\!\eta_2)\) & \(\sin(\vartheta_1\!\pm\!\vartheta_2)\) \\
Half-angle addition & \(\tanh\frac{\eta_1\!\pm\!\eta_2}{2}\) &
\(\displaystyle\frac{\tan\frac{\vartheta_1}{2}\pm \tan\frac{\vartheta_2}{2}}{1\mp \tan\frac{\vartheta_1}{2}\tan\frac{\vartheta_2}{2}}\) \\
Transverse Doppler & \(\sqrt{1-\beta^2}\) & \(\cos\vartheta\) \\
Aberration (1D) & \(\displaystyle\cos\theta'=\frac{\cos\theta-\beta}{1-\beta\cos\theta}\) &
\(\displaystyle\tan\frac{\theta'}{2}=\frac{\tan\frac{\theta}{2}-\tan\frac{\vartheta}{2}}{1+\tan\frac{\theta}{2}\tan\frac{\vartheta}{2}}\) \\
\hline
\end{tabular}
\end{center}

\subsection*{``Gudermannian bridge'': a short proof}
Define \(\vartheta\) by any one of the equivalent relations
\begin{equation}
\sin\vartheta=\tanh\eta,\qquad \cos\vartheta=\operatorname{sech}\eta,\qquad
\tan\frac{\vartheta}{2}=\tanh\frac{\eta}{2},\qquad \vartheta=\operatorname{gd}(\eta).
\end{equation}
Then (i) \(\sin^2\vartheta+\cos^2\vartheta=\tanh^2\eta+\operatorname{sech}^2\eta=1\) (since \(1-\tanh^2=\operatorname{sech}^2\)),
and (ii) differentiating gives
\begin{equation}
\frac{\mathrm{d}\vartheta}{\mathrm{d}\eta} = \operatorname{sech}\eta = \cos\vartheta.
\end{equation}
Therefore \(\gamma=\cosh\eta=\sec\vartheta\), \(\beta=\tanh\eta=\sin\vartheta\), and
all entries in the correspondence table follow from standard circular identities applied to \(\vartheta\).

\bigskip

\section{Quaternionic notation quick reference}
\label{app:quat-quick-ref}
We summarize the algebra and the sign conventions used in the manuscript.
Quaternions are written \(q=q_0+\mathbf{q}\) with scalar part \(q_0\in\mathbb{R}\) and vector part
\(\mathbf{q}=q_1\,\mathbf{i}+q_2\,\mathbf{j}+q_3\,\mathbf{k}\).

\subsection*{Units and multiplication}
\begin{equation}
\mathbf{i}^2=\mathbf{j}^2=\mathbf{k}^2=\mathbf{i}\mathbf{j}\mathbf{k}=-1,\qquad
\mathbf{i}\mathbf{j}=\mathbf{k},\ \mathbf{j}\mathbf{k}=\mathbf{i},\ \mathbf{k}\mathbf{i}=\mathbf{j},
\end{equation}
and the products reverse with a minus sign, e.g.\ \(\mathbf{j}\mathbf{i}=-\mathbf{k}\).
For \emph{pure vectors} \(\mathbf{a},\mathbf{b}\in\mathbb{R}^3\subset\mathrm{Im}\,\mathbb{H}\) one has
\begin{equation}
\mathbf{a}\,\mathbf{b} = -\,\mathbf{a}\!\cdot\!\mathbf{b} \;+\; \mathbf{a}\times\mathbf{b}.
\end{equation}

\subsection*{Conjugation, norm, inverse}
\begin{equation}
\bar q = q_0 - \mathbf{q},\qquad \|q\|^2 = q\bar q = \bar q\,q = q_0^2+\|\mathbf{q}\|^2,\qquad
q^{-1}=\frac{\bar q}{\|q\|^2}\quad(\|q\|\neq 0).
\end{equation}

\subsection*{Unit quaternions and rotations}
Let \(\widehat{\mathbf{n}}\) be a unit 3–vector and \(\theta\in\mathbb{R}\).
A unit quaternion is \(u=\exp(\tfrac{\theta}{2}\,\widehat{\mathbf{n}})=\cos\frac{\theta}{2}+\widehat{\mathbf{n}}\sin\frac{\theta}{2}\).
It rotates a pure vector \(\mathbf{v}\) by
\begin{equation}
\mathbf{v}' = u\,\mathbf{v}\,u^{-1},
\end{equation}
right–handed about \(\widehat{\mathbf{n}}\) by angle \(\theta\).

\subsection*{D-rotation (unimetry boost)}
Let \(\uhat\) be a unit spatial direction. A D-rotation by phase angle \(\psi\) is the unit rotor
\[
d=\cos\frac{\psi}{2}+\uhat\,\sin\frac{\psi}{2},\qquad d\,\bar d=1,
\]
acting on a unimetry 4-object \(\mathbf q\) by the same-sense double-sided map
\[
\boxed{\;\mathbf q' \;=\; d\,\mathbf q\,d\;}.
\]
Kinematics: \(\beta\equiv \|\mathbf v\|/\clight=\sin\psi,\ \gamma=\sec\psi\), hence
\(\displaystyle \tan\frac{\psi}{2}=\frac{\gamma\beta}{\gamma+1}\).
Composition of non-collinear D-rotations produces a residual spatial Wigner rotation \(r_W\) (see Sec.~\ref{subsec:wigner}).



\subsection*{Differentials and 1–forms (Maurer–Cartan)}
For a unit quaternion field \(u(x)\),
\begin{equation}
\Omega_L := u^{-1}\mathrm{d}u,\qquad \Omega_R := \mathrm{d}u\,u^{-1},
\end{equation}
are pure–vector imaginary 1–forms (left/right Maurer–Cartan forms).
For small \(\delta\theta\) about axis \(\widehat{\mathbf{n}}\), \(\Omega_L\sim \tfrac{1}{2}\,\widehat{\mathbf{n}}\,\delta\theta\).

\subsection*{Notation used in the text}
\begin{itemize}
\item Hats \(\widehat{\cdot}\) denote normalization to unit quaternions/vectors.
\item Tildes \(\tilde{\cdot}\) label proto–space quantities (``Khóra'' velocities, etc.).
\item Dots \(\dot{\cdot}\) denote derivatives with respect to local/proper time.
\item Levi–Civita symbol \(\varepsilon_{123}=+1\), metric signature is \((-+++) \).
\end{itemize}
