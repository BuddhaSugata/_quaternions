...
A unit rotor $\widehat q(\vartheta,\mathbf u)=\cos\vartheta+\mathbf u\sin\vartheta$ encodes motion by a \emph{circular} angle $\vartheta$, establishing the dictionary
...
Extending to curved backgrounds, we introduce a gravitational \emph{clock angle} $\phi$ via the tetrad time leg ($\cos\phi=\sqrt{-g_{00}}$ in stationary cases) and obtain a unified time-rate factorization
