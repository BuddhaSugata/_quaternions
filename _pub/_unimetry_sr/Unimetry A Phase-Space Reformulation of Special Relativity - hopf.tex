...
\subsection{Natural derivation of $\nu = k\,\tH$ from the $S^3$ Hopf torus}\label{subsec:nu_kH_hopf}
We parametrize the 3-sphere $S^3_R\subset\mathbb R^4$ by angles $(\xi_0,\xi_1,\xi_2)$:
\begin{align*}
x_1&=R\cos\xi_0\cos\xi_1, & x_2&=R\cos\xi_0\sin\xi_1,\\
x_3&=R\sin\xi_0\cos\xi_2, & x_4&=R\sin\xi_0\sin\xi_2,
\end{align*}
which induces the metric
\[
ds^2=R^2\big(d\xi_0^2+\cos^2\xi_0\,d\xi_1^2+\sin^2\xi_0\,d\xi_2^2\big).
\]
Fixing $\xi_0=\xi_0^\star$ produces a Hopf torus $\mathbb T^2\subset S^3$ with radii
\[
R_1=R\cos\xi_0^\star,\qquad R_2=R\sin\xi_0^\star,
\]
and flat induced metric $ds^2\big|_{\xi_0^\star}=R_1^2\,d\xi_1^2+R_2^2\,d\xi_2^2$.
On this torus the free (geodesic) Lagrangian is
\[
L=\tfrac12\big(R_1^2\dot\xi_1^{\,2}+R_2^2\dot\xi_2^{\,2}\big),
\]
so that $\xi_1,\xi_2$ are cyclic coordinates with conserved momenta
\[
p_1=R_1^2\dot\xi_1=\text{const},\qquad p_2=R_2^2\dot\xi_2=\text{const}.
\]
Introduce arc-length variables along the two circles,
\[
\chi=R_1\xi_1,\qquad \zeta=R_2\xi_2,\qquad \dot\chi=R_1\dot\xi_1,\quad \dot\zeta=R_2\dot\xi_2,
\]
and choose the internal-time gauge of unimetry by fixing the speed along the ``time'' circle:
\[
\dot\zeta\equiv\tH .
\]
Then
\[
\frac{p_1}{p_2}
=\frac{R_1^2\dot\xi_1}{R_2^2\dot\xi_2}
=\frac{R_1}{R_2}\,\frac{\dot\chi}{\dot\zeta}
=\frac{R_1}{R_2}\,\frac{\nu}{\tH}
\quad\Rightarrow\quad
\boxed{\ \nu=k\,\tH,\qquad k:=\frac{p_1}{p_2}\,\frac{R_1}{R_2}\ }\,. \label{eq:nu_kH_hopf}
\]
In the isotropic rest calibration $p_1=p_2$ this reduces to $k=R_1/R_2$. For the Clifford torus $\xi_0^\star=\pi/4$ one has $R_1=R_2$ and hence $k=1$.
\paragraph{GR outlook.}
In curved settings (slow spatial variation of $\xi_0(x)$ and nontrivial Hopf connection) the radii $R_{1,2}(x)$ and the ratio $k(x)$ become fields. The local clock rate
\[
\nu(x)=k(x)\,\tH
\]
then encodes gravitational time dilation as a purely geometric effect of the torus radii/connection in the unimetry framework.
\paragraph{Notation.}
We deliberately use $(\xi_0,\xi_1,\xi_2)$ to avoid conflicts with previously used angles $(\beta,\eta,\alpha)$ elsewhere in the manuscript.
