...
# --- Curvature (FRW) ---
OMEGA_K = 0.0   # = 0 (плоская), >0 (открытая), <0 (закрытая, 3-сфера)
...
c_km_s = 299792.458
H0 = 70.0  # оставь как у тебя выше, если уже определён

def E_z(z, Om, Ok, Ol):
    """E(z) = H(z)/H0 for FRW with curvature."""
    z = np.asarray(z, float)
    return np.sqrt(Om*(1+z)**3 + Ok*(1+z)**2 + Ol)

def DC_dimless(z, Om, Ok, Ol, Nint=800):
    """D_C/H0/c in *dimensionless* units: ∫0^z dz'/E(z')."""
    z = float(z)
    if z <= 0: return 0.0
    zg = np.linspace(0.0, z, Nint)
    Ez = E_z(zg, Om, Ok, Ol)
    # без (c/H0): чисто безразмерный D_C
    return np.trapz(1.0/np.maximum(Ez, 1e-12), zg)

def S_k(x, Ok):
    """Transverse comoving distance kernel S_k(x) in dimensionless units."""
...
def Skp_over_Sk(x, Ok):
    """Ratio S_k'(x)/S_k(x), well-behaved at small x."""
    tiny = 1e-12
    if np.isclose(Ok, 0.0):
        return 1.0/max(x, tiny)
    s = np.sqrt(abs(Ok))
    if Ok > 0:
        # d/dx sinh(sx)/s = cosh(sx); divide by S_k: cosh(sx)/[(sinh(sx))/s] = s*coth(sx)
        sx = s*x
        return s*np.cosh(sx)/max(np.sinh(sx), tiny)  # = s*coth(sx)
    else:
        # d/dx sin(sx)/s = cos(sx); divide by S_k: cos(sx)/[(sin(sx))/s] = s*cot(sx)
        sx = s*x
