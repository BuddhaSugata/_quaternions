...
\paragraph{Quaternionic pullback as angle compensation.}
Let $d_1$ and $d_2$ be the $D$-rotors of two successive boosts. Their raw action on any
unimetry 4-object is
\begin{equation}
\mathbf q' \;=\; d_2 d_1\, \mathbf q\, d_1 d_2 .
\label{eq:raw-two-boosts}
\end{equation}
Let $d_{12}$ denote the unique $D$-rotor that reproduces the combined space--time tilt
(the change of the spatio--temporal angle) of $d_2 d_1$, i.e.\ it maps the time axis to
the composite 3-velocity given by the velocity-addition law in \S\ref{sec:vel-addition}.
Pulling back by the conjugate rotor $\bar d_{12}$ on both sides removes this tilt:
\begin{equation}
\mathbf q^{(u)} \;=\; \bar d_{12}\, \mathbf q' \,\bar d_{12}
\;=\; (\bar d_{12} d_2 d_1)\, \mathbf q\, (d_1 d_2 \bar d_{12}).
\label{eq:pullback-angle-comp}
\end{equation}
Define the residual rotor
\begin{equation}
\boxed{\, r_W \;\equiv\; \bar d_{12}\, d_2 d_1 \,}
\qquad\Rightarrow\qquad
\mathbf q^{(u)} \;=\; r_W\, \mathbf q\, r_W^{-1},
\label{eq:rW-def}
\end{equation}

\paragraph{Uniqueness of $d_{12}$ and factorization lemma.}
Let $\mathbf e_t$ denote the unit temporal basis (observer's time axis). The composite
$D$-rotor $d_{12}$ is uniquely fixed by requiring it to map the time axis to that of the
product $d_2 d_1$,
\begin{equation}
d_{12}\,\mathbf e_t\, d_{12} \;=\; d_2 d_1\, \mathbf e_t\, d_1 d_2,
\qquad \text{with }\ \Re(d_{12})\ge 0,
\label{eq:d12-uniqueness}
\end{equation}
(the sign choice $\Re(d_{12})\ge 0$ removes the trivial two-fold ambiguity $d\to -d$).

\paragraph{Lemma (D--R factorization).}
For any product of $D$-rotors $L=d_2 d_1$ there exist unique rotors
$d_{12}$ (of type $D$) and $r_W$ (of type $R$) such that
\begin{equation}
\boxed{\; L \;=\; d_{12}\, r_W \;},\qquad
r_W \;=\; \bar d_{12}\, L \;=\; \bar d_{12}\, d_2 d_1.
\label{eq:DR-polar}
\end{equation}

\paragraph{Order dependence (noncommutativity).}
Write
\begin{equation}
L_{12}\equiv d_2 d_1 \;=\; d_{12}\, r_W,\qquad
L_{21}\equiv d_1 d_2 \;=\; d_{21}\, r_W^{-1},
\label{eq:order-L}
\end{equation}
where $d_{12}$ and $d_{21}$ are the unique $D$-rotors specified by
\eqref{eq:d12-uniqueness} for $L_{12}$ and $L_{21}$, respectively, and $r_W$ is the
Wigner rotor defined in \eqref{eq:rW-def}. In general,
\begin{equation}
d_{12}\neq d_{21},\qquad r_W^{-1}\neq r_W,
\end{equation}
except for the collinear (or trivial) case. Swapping the order flips the Wigner
rotation ($r_W\mapsto r_W^{-1}$) and changes the net 3-velocity, reflecting the
noncommutativity of velocity addition ($\mathbf v_2\oplus \mathbf v_1\neq
\mathbf v_1\oplus \mathbf v_2$). The quaternionic compensation picture remains the
same: for the $(1\!\to\!2)$ order the observed map is $r_W\,\mathbf q\,r_W^{-1}$,
for $(2\!\to\!1)$ it is $r_W^{-1}\,\mathbf q\,r_W$.



\begin{center}
\fbox{\begin{minipage}{0.94\linewidth}
\textbf{One-line algorithm for $r_W$.}
Compute $L=d_2 d_1$, find the unique $d_{12}$ by
$d_{12}\,\mathbf e_t\, d_{12}=L\,\mathbf e_t\,L$ with $\Re(d_{12})\ge 0$,
then $r_W=\bar d_{12}\,L$ and the observed action is
