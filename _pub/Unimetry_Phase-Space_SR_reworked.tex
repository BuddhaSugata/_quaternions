...
\usepackage{amsfonts}
\usepackage{booktabs}
% Bookmark-safe math mappings for hyperref
\makeatletter
...
We define the \emph{phase angle} $\vartheta\in[0,\tfrac{\pi}{2})$ of a system with respect to a fixed observer as an \textbf{operationally measurable split} of a constant ``flow budget'' $c$ between temporal and spatial projections:
\begin{equation}
...
The phase angle $\vartheta$ is \emph{not} the rapidity $\eta$; they are related by a Gudermann-type bridge
\begin{equation}
...
\end{equation}
where $R_W\in\mathrm{SO}(3)$ is the Wigner--Thomas rotation (a genuine 3D rotation), while $q(\vartheta_{12},\mathbf u_{12})$ is the effective boost in the slice $\mathbb C_{\mathbf u_{12}}$. The rotation angle and axis can be extracted from the vector part of the quaternion product; a closed expression equivalent to the standard formulas is provided in Appendix~\ref{app:wigner}.
...
% === New Section: From SR to GR ===
\section{\texorpdfstring{From SR to GR: local tetrads and gravitational redshift}{From SR to GR}}
On a curved background $(\mathcal{M},g)$ we work with orthonormal tetrads $e^{a}{}_{\mu}(x)$, $g_{\mu\nu}e^{\mu}{}_{a}e^{\nu}{}_{b}=\eta_{ab}$. In each tangent space the same phase-rotation picture applies. The temporal projection measured by a stationary observer with time-leg $e^{0}{}_{\mu}$ is
\begin{equation}
\cos\vartheta \ \propto\ e^{0}{}_{\mu}\,u^{\mu}=\frac{d\tau}{dt}\ =\ \sqrt{-g_{00}}\quad\text{(stationary case)},
\end{equation}
hence the gravitational redshift between levels $\Phi_1,\Phi_2$ in the weak-field limit $g_{00}\approx -(1+2\Phi/c^2)$ becomes
\begin{equation}
\frac{\nu_2}{\nu_1}=\frac{\cos\vartheta_2}{\cos\vartheta_1}\ \approx\ \frac{1+\Phi_2/c^2}{1+\Phi_1/c^2}\ \approx\ 1+\frac{\Phi_2-\Phi_1}{c^2}.
\end{equation}
Uniform acceleration (Rindler) accumulates phase as $d\vartheta=(a/c)\,d\tau$, yielding $\vartheta(\tau)=\eta(\tau)$ along the worldline and reproducing the standard kinematics of accelerated frames.
...
\textbf{Data Availability} — Analytical derivations only. A public preprint is available at Zenodo (DOI: 10.5281/zenodo.17143555). No datasets or code were generated.\\
\textbf{Authors’ Contributions} — T.A.: conceptualization, formal analysis, writing—original draft and review.
